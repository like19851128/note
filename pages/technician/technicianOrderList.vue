			...
			<swiper-item class="tab-content" v-for="(tabItem,tabIndex) in navList" :key="tabIndex">
				...
				>
					...
					>
						...
						<view class="action-box b-t" v-if="[1, 2, 3, 4, 5].includes(item.orderStatusInt) && item.operationRole !== 'SYS'">
							<button v-if="item.orderStatusInt==1" class="action-btn" @click.stop="operationOrder(item, 'JS_ARRAY_GET', '接单')">接单</button>
							<button v-if="item.orderStatusInt==2" class="action-btn" @click.stop="operationOrder(item, 'JS_SET_OUT', '出发')">出发</button>
							<button v-if="item.orderStatusInt==3" class="action-btn" @click.stop="operationOrder(item, 'JS_ARRIVE', '到达')">到达</button>
							<button v-if="item.orderStatusInt==4" class="action-btn" @click.stop="operationOrder(item, 'JS_SERVICE_START', '开始服务')">开始服务</button>
							<button v-if="item.orderStatusInt==5" class="action-btn" @click.stop="operationOrder(item, 'JS_SERVICE_END', '服务完成')">服务完成</button>
						</view>
					</view>
	...
	export default {
			...
			refreshOrderList() {
				try {
					...
					currentPagsList.forEach(item => {
						if (item.route=='pages/technician/technicianOrderList') {
							backPage = item
						}
					})
					if (!backPage) return
		...
		methods: {
			...
			loadData(source){
				...
				this.$api.artificerOrderPageList(params).then(res => {
			...
			toDetail(item) {
				uni.navigateTo({
					url: `/pages/technician/technicianOrderDetail?orderNo=${item.orderNo}`
				})
				console.log(item)
			},
			//swiper 切换
