...
let uploadPath, uploadPathSY;
// #ifdef MP-WEIXIN
uploadPath = utils.baseURL + '/file/upload'; //上传地址（不加水印）
uploadPathSY = utils.baseURL + '/local-file/upload'; //上传地址（加水印）
// #endif

// #ifdef H5
uploadPath = 'https://ylm.6plus7.cn/file/upload'; //上传地址（不加水印）
uploadPathSY = 'https://ylm.6plus7.cn/local-file/upload'; //上传地址（加水印）
// #endif
...
export default {
		...
		return new Promise((resolve) => {
			if (file.indexOf("blob:") > -1 || file.indexOf("file:") > -1 || file.indexOf('http://tmp/') > -1) {
				const uploadTask = uni.uploadFile({
					url: _this.sy ? uploadPathSY : uploadPath,
					filePath: file,
					...
					success: (uploadFileRes) => {
						...
						*/
						// let _res=uploadFileRes.data.data;           //按照你的实际响应修改
						let _res = JSON.parse(uploadFileRes.data).data; //按照你的实际响应修改
	...
	ergodicObject(obj) {
		...
		Object.keys(obj).forEach(x => {
			_styStr += x.replace(reg, "-$1").toLowerCase() + ':' + obj[x] + ';'; //驼峰式命名转CSS连字符
		})
		return _styStr;
		// #endif
		// #ifndef MP
